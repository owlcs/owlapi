...
public class MapPointer<K, V extends OWLAxiom> {
  ...
  public synchronized MapPointer<K, V> init() {
    ...
    if (visitor instanceof InitVisitor) {
      i.getAxiomsByType()
          .forEach(verifyNotNull(type), ax -> putInternal(ax.accept((InitVisitor<K>) verifyNotNull(
              visitor)), (V) ax));
    } else if (visitor instanceof InitCollectionVisitor) {
      i.getAxiomsByType()
          .forEach(verifyNotNull(type), ax -> ax.accept((InitCollectionVisitor<K>) verifyNotNull(
