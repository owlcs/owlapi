...
Features
Major reworking of the osgi bundle. Everything is embedded transitively, except for slf4j-api.
Formats have suggested extensions as well.
Simplify the trivially transitive check
Support preserving, transferring, and adding annotations during OBO Macro expansion
Allow ELK to be used with protege's classification result display panel. Some reasoners may not support OWLReasoner::isEntailed.
Use custom trove hashmap and hashset, with a strategy that checks for hashcode equality before trying full equality check
Configuration option to skip validation before writing OBO file #290

Bug fixes
Amend factory registry to allow multithread manipulation
Fix imports in OWL Manchester Notation fail #347
Solve some thread safety issues in XML parsers and storers by using thread locals instead of statics.
...
Fix javadoc bug in oboformat  (hard error with jdk8 )
Fix #297 ManchesterSyntax rendering prints annotations twice.
Fix #292 SWRL rules saved by older versions of OWLAPI/Protege lose annotations
Removed nondeterminism from facet order
...
Fix #282 Bug with compareTo in 3.5.1
...
feature: OBO parser replaced with oboformat. Original code at: https://code.google.com/p/oboformat/
