...
import org.semanticweb.owlapi6.utilities.Version;
 ...
 * using the {@link OWLReasoner#getRootOntology()} method. When the client responsible for creating
 * the reasoner has finished with the reasoner instance it must call the {@link #dispose()} method
 ...
 * {@link org.semanticweb.owlapi6.reasoner.OWLReasonerFactory}. <br>
 ...
 * listener to the {@link org.semanticweb.owlapi6.model.OWLOntologyManager} that manages the root
 * ontology. The reasoner will listen to any
 * {@link org.semanticweb.owlapi6.model.OWLOntologyChange}s and respond appropriately to them before
 * answering any queries. If the {@link org.semanticweb.owlapi6.reasoner.BufferingMode} of the
 * reasoner (the answer to {@link #getBufferingMode()} is
 * {@link org.semanticweb.owlapi6.reasoner.BufferingMode#NON_BUFFERING}) the ontology changes are
 ...
 * {@link org.semanticweb.owlapi6.reasoner.BufferingMode} of the reasoner is
 * {@link org.semanticweb.owlapi6.reasoner.BufferingMode#BUFFERING} then ontology changes are stored
 * in a buffer and are only taken into consideration when the buffer is flushed with the
 * {@link #flush()} method. When reasoning, axioms in the root ontology imports closure, minus the
 * axioms returned by the {@link #getPendingAxiomAdditions()} method, plus the axioms returned by
 * the {@link #getPendingAxiomRemovals()} are taken into consideration. <br>
 * Note that there is no guarantee that the reasoner implementation will respond to changes in an
 * incremental (and efficient manner) manner. <br>
 * <h2>Reasoner Axioms</h2> The set of axioms that the reasoner takes into consideration when
 * answering queries is known as the <i>set of reasoner axioms</i>. This corresponds the axioms in
 * the imports closure of the root ontology plus the axioms returned by the
 * {@link #getPendingAxiomRemovals()} minus the axioms returned by
 * {@link #getPendingAxiomAdditions()} <br>
 * <h2>Nodes</h2> The reasoner interface contains methods that return
 * {@link org.semanticweb.owlapi6.reasoner.NodeSet}s. These are sets of
 * {@link org.semanticweb.owlapi6.reasoner.Node}s. A {@code Node} contains entities. <br>
 * For a {@code Node&lt;OWLClass&gt;} of classes, each class in the node is equivalent to the other
 * classes in the {@code Node} with respect to the imports closure of the root ontology. <br>
 * For a {@code Node&lt;OWLObjectProperty&gt;} of object properties, each object property in the
 * {@code Node} is equivalent to the other object properties in the node with respect to the imports
 * closure of the root ontology. <br>
 * For a {@code Node&lt;OWLDataProperty&gt;} of data properties, each data property in the
 * {@code Node} is equivalent to the other data properties in the node with respect to the imports
 * closure of the root ontology. <br>
 * For a {@code Node&lt;OWLNamedIndividual&gt;} of named individuals, each individual in the node is
 ...
 * hierarchy has a top node (see org.semanticweb.owlapi6.reasoner.Node#isTopNode()) and a bottom
 * node (see {@link org.semanticweb.owlapi6.reasoner.Node#isBottomNode}). <br>
 ...
 * along with {@code owl:Thing}. Similarly, class {@code K} is unsatisfiable, so it is equivalent to
 * {@code owl:Nothing}, and therefore appears in the bottom node containing {@code owl:Nothing}. In
 * this example, classes {@code A} and {@code B} are equivalent so they appear in one node, also,
 * classes {@code D} and {@code F} are equivalent so they appear in one node. <br>
 * Asking for the subclasses of a given class (expression) returns the a {@code NodeSet} containing
 * the nodes that contain classes that are strict subclasses of the specified class (expression).
 * For example, asking for the subclasses of class {@code C} returns the {@code NodeSet}
 * {@code &#123;E&#125;} and {@code &#123;owl:Nothing, K&#125;}. <br>
 * Asking for the direct subclasses of a given class (expression) returns the {@code NodeSet} that
 * contains the nodes that contains classes that are direct subclasses of the specified class. For
 * example, asking for the direct subclasses of class {@code A} returns the {@code NodeSet}
 * containing the nodes {@code &#123;C&#125;} and {@code &#123;D, F&#125;}. Note that there are
 * convenience methods on {@link NodeSet} and {@link org.semanticweb.owlapi6.reasoner.Node} that can
 * be used to directly access the entities in a {@code NodeSet} without having to iterate over the
 * nodes and entities in a {@code NodeSet}. For example, a "plain" stream of classes contained
 * inside the {@code Nodes} contained inside a {@code NodeSet} can easily be obtained using the
 * {@link NodeSet#entities()} method. In this case we could quickly obtain
 * {@code &#123;C, D, F&#125;} as the direct subclasses of {@code A} simply by using the
 * {@link #getSubClasses(org.semanticweb.owlapi6.model.OWLClassExpression, boolean)} (with
 ...
 * {@code Node} {@code &#123;owl:Nothing, K&#125;}. <br>
 * <img src="../../../../doc-files/hierarchy.png" alt="hierarchy"><br>
 * <h2>Definitions</h2> In what follows, an extension of the
 * <a href="http://www.w3.org/TR/owl2-syntax/">OWL 2 Functional Syntax</a> is given in order to
 * capture notions like a class being a "direct" subclass of another class. <br>
 * <h3>StrictSubClassOf</h3> Given two class expressions {@code CE1} and {@code CE2} and an ontology
 * {@code O}, {@code CE1} is a strict subclass of {@code CE2}, written
 * {@code StrictSubClassOf(CE1 CE2)} if {@code O} entails {@code SubClassOf(CE1 CE2)} and {@code O}
 * does not entail {@code SubClassOf(CE2 CE1)} <br>
 * <h3>DirectSubClassOf</h3> Given two class expressions {@code CE1} and {@code CE2} and an ontology
 * {@code O}, {@code CE1} is a <i>direct</i> subclass of {@code CE2}, written
 * {@code DirectSubClassOf(CE1 CE2)}, with respect to {@code O} if {@code O} entails
 * {@code StrictSubClassOf(CE1 CE2)} and there is no class name {@code C} in the signature of
 * {@code O} such that {@code O} entails {@code StrictSubClassOf(CE1 C)} and {@code O} entails
 * {@code StrictSubClassOf(C CE2)}. <br>
 * <h3>StrictSubObjectPropertyOf</h3> Given two object property expressions {@code OPE1} and
 * {@code OPE2} and an ontology {@code O}, {@code OPE1} is a strict subproperty of {@code OPE2},
 ...
 * <h3>DirectSubObjectPropertyOf</h3> Given two object property expressions {@code OPE1} and
 * {@code OPE2} and an ontology {@code O}, {@code OPE1} is a <i>direct</i> subproperty of
 * {@code OPE2}, written {@code DirectSubObjectPropertyOf(OPE1 OPE2)}, with respect to {@code O} if
 * {@code O} entails {@code StrictSubObjectPropertyOf(OPE1 OPE2)} and there is no object property
 ...
 * <h3>DirectSubDataPropertyOf</h3> Given two data property expressions {@code DPE1} and
 * {@code DPE2} and an ontology {@code O}, {@code DPE1} is a <i>direct</i> subproperty of
 * {@code DPE2}, written {@code DirectSubDataPropertyOf(DPE1 DPE2)}, with respect to {@code O} if
 * {@code O} entails {@code StrictSubDataPropertyOf(DPE1 DPE2)} and there is no data property name
 * {@code P} in the signature of {@code O} such that {@code O} entails
 * {@code StrictSubDataPropertyOf(DPE1 P)} and {@code O} entails
 * {@code StrictSubDataPropertyOf(P DPE2)}. <br>
 * <h3>DirectClassAssertion</h3> Given an individual {@code j} and a class expression {@code CE} and
 * an ontology {@code O}, {@code CE} is a direct class assertion (type) for {@code j}, written
 * {@code DirectClassAssertion(CE j)}, if {@code O} entails {@code ClassAssertion(CE j)} and there
 * is no class name {@code C} in the signature of {@code O} such that {@code O} entails
 * {@code ClassAssertion(C j)} and {@code O} entails {@code StrictSubClassOf(C CE)}.
 * <h3>ObjectPropertyComplementOf</h3> Given an object property expression {@code pe}, the object
 * property complement of {@code pe} is written as {@code ObjectPropertyComplementOf(pe)}. The
 * interpretation of {@code ObjectPropertyComplementOf(pe)} is equal to the interpretation of
 * {@code owl:topObjectProperty} minus the interpretation of {@code pe}. In other words,
 * {@code ObjectPropertyComplementOf(pe)} is the set of pairs of individuals that are not in
 * {@code pe}.
 * <h3>DataPropertyComplementOf</h3> Given a data property expression {@code pe} , the data property
 * complement of {@code pe} is written as {@code DataPropertyComplementOf(pe)}. The interpretation
 * of {@code DataPropertyComplementOf(pe)} is equal to the interpretation of
 ...
 * <h2>Error Handling</h2> An {@code OWLReasoner} may throw the following exceptions to indicate
 * errors. More documentation for each type of exception can be found on the particular exception
 * class.
 * <ul>
 * <li>{@link org.semanticweb.owlapi6.reasoner.AxiomNotInProfileException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.ClassExpressionNotInProfileException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.FreshEntitiesException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.InconsistentOntologyException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.TimeOutException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.ReasonerInterruptedException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.UnsupportedEntailmentTypeException}</li>
 * <li>{@link org.semanticweb.owlapi6.reasoner.ReasonerInternalException}</li>
 * </ul>
 * Note that {@link org.semanticweb.owlapi6.reasoner.ReasonerInternalException} may be throw by any
...
public interface OWLReasoner {
     ...
     * consideration the changes the current root ontology specified by the changes. If the reasoner
     * buffering mode is {@link org.semanticweb.owlapi6.reasoner.BufferingMode#NON_BUFFERING} then
     ...
     * {@link org.semanticweb.owlapi6.reasoner.BufferingMode#NON_BUFFERING} then there will be no
     ...
     * {@link #getPendingAxiomRemovals()}, minus the axioms returned by
     * {@link #getPendingAxiomAdditions()} when reasoning. <br>
     * Note that the root ontology is set at reasoner creation time and cannot be changed
     * thereafter. Clients that want to add ontologies to and remove ontologies from the reasoner
     * after creation time should create a "dummy" ontology that imports the "real" ontologies and
    ...
    /**
     * Asks the reasoner to interrupt what it is currently doing. An ReasonerInterruptedException
    ...
    /**
     * Asks the reasoner to precompute certain types of inferences. Note that it is NOT necessary to
     * call this method before asking any other queries - the reasoner will answer all queries
     * correctly regardless of whether inferences are precomputed or not. For example, if the
     * imports closure of the root ontology entails {@code SubClassOf(A B)} then the result of
     * {@code getSubClasses(B)} will contain {@code A}, regardless of whether
     * {@code precomputeInferences( InferenceType#CLASS_HIERARCHY)} has been called. <br>
     * If the reasoner does not support the precomputation of a particular type of inference then it
    ...
    /**
     * Returns the set of {@link org.semanticweb.owlapi6.reasoner.InferenceType}s that are
     * precomputable by reasoner.
     *
     * @return A set of {@link org.semanticweb.owlapi6.reasoner.InferenceType}s that can be
     *         precomputed by this reasoner.
     */
    Set<InferenceType> getPrecomputableInferenceTypes();

    /**
     * Returns the stream of {@link org.semanticweb.owlapi6.reasoner.InferenceType}s that are
     * precomputable by reasoner.
     *
     * @return A stream of {@link org.semanticweb.owlapi6.reasoner.InferenceType}s that can be
     *         precomputed by this reasoner.
     */
     ...
     * an {@link org.semanticweb.owlapi6.reasoner.InconsistentOntologyException} even if the root
     * ontology imports closure is inconsistent.
     *
     ...
     *         by this reasoner.
     * @throws InconsistentOntologyException if the set of reasoner axioms is inconsistent
     * @see #isEntailmentCheckingSupported(org.semanticweb.owlapi6.model.AxiomType)
     */
     ...
     *         {@link #isEntailed(org.semanticweb.owlapi6.model.OWLAxiom)} will <em>not</em> throw
     *         an exception of
     *         {@link org.semanticweb.owlapi6.reasoner.UnsupportedEntailmentTypeException} . If
     *         {@code false} then asking {@link #isEntailed(org.semanticweb.owlapi6.model.OWLAxiom)}
     *         <em>will</em> throw an
     *         {@link org.semanticweb.owlapi6.reasoner.UnsupportedEntailmentTypeException} .
     */
     ...
     *         calling
     *         {@link #getEquivalentClasses(org.semanticweb.owlapi6.model.OWLClassExpression)} with
     ...
     * returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * that the classes are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * {@link org.semanticweb.owlapi6.reasoner.Node}.
     *
     ...
     * classes are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     *         {@link #getEquivalentObjectProperties(org.semanticweb.owlapi6.model.OWLObjectPropertyExpression)}
     ...
     * {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     * @param pe The object property expression whose strict (direct) subproperties are to be
     *        retrieved.
     * @param direct Specifies if the direct subproperties should be retrived ( {@code true}) or if
     ...
     * properties are returned as a {@link org.semanticweb.owlapi6.reasoner.Node}.
     *
     ...
     * axioms. The properties are returned as a {@link org.semanticweb.owlapi6.reasoner.Node}.
     *
     * @param pe The object property expression whose equivalent properties are to be retrieved.
     * @return A node containing the <a href="#spe">simplified object property expressions</a> such
     *         that for each <a href="#spe">simplified object property expression</a>, {@code P}, in
     ...
     *         returned by calling
     *         {@link #getEquivalentDataProperties(org.semanticweb.owlapi6.model.OWLDataProperty)}
     ...
     *      the properties are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     */
     ...
     *      properties are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     */
     ...
     * returned as a {@link org.semanticweb.owlapi6.reasoner.Node}.
     *
     ...
     * individual. The classes are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     *      as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     */
     ...
     * are returned a a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * @throws TimeOutException if the reasoner timed out during a basic reasoning operation. See
     *         {@link #getTimeOut()}.
     * @see org.semanticweb.owlapi6.reasoner.IndividualNodeSetPolicy
     */
     ...
     * The individuals are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * individuals are returned as a {@link org.semanticweb.owlapi6.reasoner.NodeSet}.
     *
     ...
     * reasoner creation time. During satisfiability (subsumption) checking the reasoner will check
     * to see if the time it has spent doing the single check is longer than the value returned by
     * this method. If this is the case, the reasoner will throw a
     * {@link org.semanticweb.owlapi6.reasoner.TimeOutException} in the thread that is executing the
    ...
    /**
     * Gets the Fresh Entity Policy in use by this reasoner. The policy is set at reasoner creation
     * time.
     *
     * @return The policy.
     */
    ...
    /**
     * Gets the IndividualNodeSetPolicy in use by this reasoner. The policy is set at reasoner
     * creation time.
     *
     * @return The policy.
     */
     ...
     * reasoner as an {@link org.semanticweb.owlapi6.model.OWLOntologyChangeListener} from the
     * {@link org.semanticweb.owlapi6.model.OWLOntologyManager} that manages the ontologies
