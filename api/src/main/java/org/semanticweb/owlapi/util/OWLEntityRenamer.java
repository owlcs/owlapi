 ...
 * regardless of whether they are classes, object properties, data properties, individuals or data
 * types.
 * 
...
public class OWLEntityRenamer {
    ...
    /**
     * Changes a IRI for another IRI. This creates the appropriate changes to be applied in order to
     * change a IRI.
     * 
     * @param iri The IRI to be changed
     * @param newIRI The IRI that the IRI should be changed to.
     * @return A list of ontology changes that should be applied to change the specified IRI.
     */
    @Nonnull
    public List<OWLOntologyChange> changeIRI(@Nonnull IRI iri, @Nonnull IRI newIRI) {
        ...
        OWLObjectDuplicator dup =
            new OWLObjectDuplicator(owlOntologyManager.getOWLDataFactory(), uriMap);
     ...
     * @return A list of ontology changes that should be applied to change the specified entity IRI.
     */
    @Nonnull
    public List<OWLOntologyChange> changeIRI(@Nonnull OWLEntity entity, @Nonnull IRI newIRI) {
        ...
        OWLObjectDuplicator duplicator =
            new OWLObjectDuplicator(iriMap, owlOntologyManager.getOWLDataFactory());
        for (OWLOntology ont : ontologies) {
            assert ont != null;
    ...
    /**
     * @param entity2IRIMap map of IRIs to rename
     * @return list of changes
     */
    public List<OWLOntologyChange> changeIRI(@Nonnull Map<OWLEntity, IRI> entity2IRIMap) {
        List<OWLOntologyChange> changes = new ArrayList<>();
        OWLObjectDuplicator duplicator =
            new OWLObjectDuplicator(entity2IRIMap, owlOntologyManager.getOWLDataFactory());
        for (OWLOntology ont : ontologies) {
            assert ont != null;
    ...
    private static void fillListWithTransformChanges(List<OWLOntologyChange> changes,
        Set<OWLAxiom> axioms, @Nonnull OWLOntology ont, OWLObjectDuplicator duplicator) {
    ...
    private static void fillListWithOntologyAnnotationTransformChanges(
        List<OWLOntologyChange> changes, @Nonnull OWLOntology ont, OWLObjectDuplicator duplicator) {
        for (OWLAnnotation ann : ont.getAnnotations()) {
            OWLAnnotation dupAx = duplicator.duplicateObject(ann);
            if (!dupAx.equals(ann)) {
