...
public class SmallSet<T> extends AbstractSet<T> {
  ...
  public SmallSet(Collection<T> collection) {
    if (collection.size() > 3) {
      throw new IllegalArgumentException(
          "Trying to create a small set with too many elements - max 3, requested: " + collection
              .size());
    }
    for (T t : collection) {
      add(t);
  ...
  public boolean contains(@Nullable Object o) {
    ...
    int oHash = o.hashCode();
    return checkMatch(o, oHash, element1) || checkMatch(o, oHash, element2) || checkMatch(o, oHash,
        element3);
  }
