...
public interface LiteralProvider extends DatatypeProvider {
   ...
   * @return An OWLLiteral with the specified lexical value and specified datatype. If the datatype
   * is {@code rdf:PlainLiteral}, and the lexical value contains a language tag then the language
   * tag will be parsed out of the lexical value. For example, "abc@en"^^rdf:PlainLiteral would be
   * parsed into a lexical value of "abc" and a language tag of "en".
   */
   ...
   *
   * @param value The value of the literal
   * @return An {@code OWLTypedConstant} whose literal is the lexical value of the double, and whose
   ...
   *
   * @param value The value of the literal
   * @return An {@code OWLTypedConstant} whose literal is the lexical value of the float, and whose
   * data type is xsd:float.
   */
   ...
   * to the empty string (for backwards compatibility). If not empty, the tag is formed according to
   * <a href="http://www.rfc-editor.org/rfc/bcp/bcp47.txt">BCP47</a> but the OWL API will not check
   * that the tag conforms to this specification - it is up to the caller to ensure this.
