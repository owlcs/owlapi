 ...
 * The {@link OWLOntologyStorerFactoryRegistry} provides a central point for
 * the registration of storer factories that create storers to store OWL
 * ontologies.  The registry is typically used by at least one type of ontology
 * factory for storing ontologies whose concrete representations are to be 
 * contained in some kind of document.
 */
public class OWLOntologyStorerFactoryRegistry extends AbstractServiceLoader<OWLOntologyFormatFactory, OWLOntologyStorerFactory> {
     ...
     * @param key The format to find a storer for.
     * @return The first storer found matching this format, or null if none were found.
     */
    public OWLOntologyStorerFactory getStorerFactory(OWLOntologyFormatFactory key)
    {
        Collection<OWLOntologyStorerFactory> collection = this.get(key);
    ...
    /**
     * @param storerFactory The ontology storer factory to register
     */
    ...
    /**
     * @param storerFactory The ontology storer factory to remove
     */
