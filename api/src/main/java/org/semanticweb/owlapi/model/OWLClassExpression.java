...
public interface OWLClassExpression extends OWLObject, OWLPropertyRange, SWRLPredicate, AsOWLClass {
   ...
   *
   * @return {@code true} if this expression is owl:Nothing, or {@code false} if this expression is
   * not owl:Nothing.
   */
   ...
   *
   * @return The disjuncts of this expression if it is a disjunction (object union of), or otherwise
   * a singleton set containing this expression. Note that nested disjunctions will be flattened,
   * for example, calling this method on (A or B) or C will return the set {A, B, C}
   */
