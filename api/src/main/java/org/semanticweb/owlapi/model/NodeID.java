...
public abstract class NodeID implements Comparable<NodeID>, Serializable {
     ...
     * @param uri
     *            the IRI
     * @return {@code true} if the IRI was generated by this parser to
     *         label an anonymous node */
    public static boolean isAnonymousNodeIRI(String uri) {
        return uri != null
                && (uri.startsWith(PREFIX) || uri.indexOf(NodeID.NODE_ID_PREFIX) != -1);
    }
    ...
    public static NodeID getNodeID(String id) {
        String nonBlankId = id == null || id.length() == 0 ? PREFIX + NODE_ID_PREFIX
                + Long.toString(COUNTER.incrementAndGet()) : id;
        return new NodeIDImpl(nonBlankId);
    }
