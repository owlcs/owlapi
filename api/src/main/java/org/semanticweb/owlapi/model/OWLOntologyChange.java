...
public abstract class OWLOntologyChange implements HasSignature, Serializable {
   ...
   * from an ontology.
   *
   * @return {@code true} if the change is an {@code OWLAddAxiomChange} or {@code
   * OWLRemoveAxiomChange} otherwise {@code false}.
   */
   ...
   *
   * @return {@code true} if the change is a RemoveAxiom change and it will remove an axiom from an
   * ontology, {@code false} otherwise.
   */
   ...
   * @throws IllegalStateException if the change has no axiom; UnsupportedOperationException If the
   * change is not an axiom change (check with the {@code isAxiomChange} method first).
   */
  public OWLAxiom getAxiom() {
    throw new UnsupportedOperationException(
        "This is an " + getClass().getSimpleName() + ", not an axiom change: "
            + this);
  }
   ...
   * {@link OWLOntologyID} of this {@code OWLOntologyChange}'s {@link OWLOntology}. Not {@code null}
   * .
   */
