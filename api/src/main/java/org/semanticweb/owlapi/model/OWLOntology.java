...
public interface OWLOntology extends OWLObject, HasAnnotations, HasDirectImports, HasImportsClosure,
   ...
   * programmatically and not loaded/saved, so it does not have any format information associated.
   */
  @Nullable
   ...
   * @return A set of {@code OWLEntity} objects. The set that is returned is a copy - it will not be
   * updated if the ontology changes. It is therefore safe to apply changes to this ontology while
   * iterating over this set.
   ...
   * @return {@code true} if the ontology contains a declaration for the specified entity, otherwise
   * {@code false}.
   */
