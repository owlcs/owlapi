...
public class CreateValuePartition extends AbstractCompositeOntologyChange {
    ...
    private void generateChanges(OWLOntology targetOntology, Collection<OWLClass> valuePartitionClasses,
        ...
        for (OWLClassExpression valuePartitionValue : valuePartitionClasses) {
            addChange(new AddAxiom(targetOntology, df.getOWLSubClassOfAxiom(valuePartitionValue, valuePartitionClass)));
        }
        // 2) Make the values disjoint
        addChange(new AddAxiom(targetOntology, df.getOWLDisjointClassesAxiom(valuePartitionClasses)));
        // 3) Add a covering axiom to the value partition
        OWLClassExpression union = df.getOWLObjectUnionOf(valuePartitionClasses);
        addChange(new AddAxiom(targetOntology, df.getOWLSubClassOfAxiom(valuePartitionClass, union)));
