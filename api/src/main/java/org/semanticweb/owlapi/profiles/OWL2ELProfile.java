    ...
    protected class OWL2ELProfileObjectVisitor extends OWLOntologyWalkerVisitor {
        ...
        public void visit(OWLObjectOneOf ce) {
            if (ce.individuals().count() != 1) {
                profileViolations
                    .add(new UseOfObjectOneOfWithMultipleIndividuals(getCurrentOntology(),
                        getCurrentAxiom(), ce));
            }
        }
        ...
        public void visit(OWLDataOneOf node) {
            if (node.values().count() != 1) {
                profileViolations.add(
                    new UseOfDataOneOfWithMultipleLiterals(getCurrentOntology(), getCurrentAxiom(),
                        node));
            }
        }
        ...
        public void visit(OWLSubPropertyChainOfAxiom axiom) {
            ...
            getCurrentOntology().axioms(AxiomType.OBJECT_PROPERTY_RANGE, INCLUDED)
                .forEach(rngAx -> {
                    if (getPropertyManager()
                        .isSubPropertyOf(axiom.getSuperProperty(), rngAx.getProperty())) {
                        ...
                        if (!chain.isEmpty()) {
                            ...
                            if (!rngPresent) {
                                profileViolations.add(
                                    new LastPropertyInChainNotInImposedRange(getCurrentOntology(),
                                        axiom,
                                        rngAx));
                            }
                        }
