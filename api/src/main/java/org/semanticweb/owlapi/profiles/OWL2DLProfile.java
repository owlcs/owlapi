  ...
  private static class OWL2DLProfileObjectVisitor extends OWLOntologyWalkerVisitor {
    ...
    public void visit(OWLClass ce) {
      if (!ce.isBuiltIn() && ce.getIRI().isReservedVocabulary()) {
        profileViolations
            .add(new UseOfReservedVocabularyForClassIRI(getCurrentOntology(), getCurrentAxiom(),
                ce));
      }
      ...
      if (getCurrentOntology().containsDatatypeInSignature(ce.getIRI())) {
        profileViolations
            .add(new DatatypeIRIAlsoUsedAsClassIRI(getCurrentOntology(), getCurrentAxiom(), ce
                .getIRI()));
      }
    }
    ...
    public void visit(OWLDatatype node) {
      ...
      if (!OWL2Datatype.isBuiltIn(node.getIRI())) {
        ...
        if (!node.isTopDatatype() && !node.isBuiltIn() && !getCurrentOntology()
          ...
          profileViolations
              .add(new UseOfUndeclaredDatatype(getCurrentOntology(), getCurrentAxiom(), node));
        }
      }
      if (getCurrentOntology().containsClassInSignature(node.getIRI(), INCLUDED)) {
        profileViolations
            .add(new DatatypeIRIAlsoUsedAsClassIRI(getCurrentOntology(), getCurrentAxiom(), node
                .getIRI()));
      }
    }
    ...
    public void visit(OWLObjectProperty property) {
      if (!property.isOWLTopObjectProperty() && !property.isOWLBottomObjectProperty() && property
          .getIRI()
          .isReservedVocabulary()) {
      ...
      if (getCurrentOntology().containsDataPropertyInSignature(property.getIRI(), INCLUDED)) {
        profileViolations
            .add(new IllegalPunning(getCurrentOntology(), getCurrentAxiom(), property.getIRI()));
      }
    ...
    public void visit(OWLAnnotationProperty property) {
      ...
      if (!property.isBuiltIn() && !getCurrentOntology().isDeclared(property, INCLUDED)) {
        profileViolations
            .add(new UseOfUndeclaredAnnotationProperty(getCurrentOntology(), getCurrentAxiom(),
    ...
    public void visit(OWLSubDataPropertyOfAxiom axiom) {
      if (axiom.getSubProperty().isOWLTopDataProperty()) {
        profileViolations
            .add(new UseOfTopDataPropertyAsSubPropertyInSubPropertyAxiom(getCurrentOntology(),
                axiom));
      }
    }
    ...
    public void visit(OWLObjectHasSelf ce) {
      if (getPropertyManager().isNonSimple(ce.getProperty())) {
        profileViolations
            .add(new UseOfNonSimplePropertyInObjectHasSelf(getCurrentOntology(), getCurrentAxiom(),
                ce));
      }
    }
    ...
    public void visit(OWLFunctionalObjectPropertyAxiom axiom) {
      if (getPropertyManager().isNonSimple(axiom.getProperty())) {
        profileViolations
            .add(new UseOfNonSimplePropertyInFunctionalPropertyAxiom(getCurrentOntology(), axiom));
      }
    }
    ...
    public void visit(OWLAsymmetricObjectPropertyAxiom axiom) {
      if (getPropertyManager().isNonSimple(axiom.getProperty())) {
        profileViolations
            .add(new UseOfNonSimplePropertyInAsymmetricObjectPropertyAxiom(getCurrentOntology(),
                axiom));
      }
    }
    ...
    protected void checkExtremes(OWLSubPropertyChainOfAxiom axiom,
      ...
      if (first.equals(superProp)) {
        ...
        if (getPropertyManager().isLessThan(superProp, last)) {
          profileViolations
              .add(new UseOfPropertyInChainCausesCycle(getCurrentOntology(), axiom, last));
        }
      } else {
        // first not equal, it must be smaller
        if (getPropertyManager().isLessThan(superProp, first)) {
          profileViolations
              .add(new UseOfPropertyInChainCausesCycle(getCurrentOntology(), axiom, first));
        }
      }
    }
    ...
    protected void checkCenter(OWLSubPropertyChainOfAxiom axiom,
      ...
      for (int i = 1; i < chain.size() - 1; i++) {
        if (getPropertyManager().isLessThan(superProp, chain.get(i))) {
          profileViolations
              .add(new UseOfPropertyInChainCausesCycle(getCurrentOntology(), axiom, chain.get(
                  i)));
        }
      }
    }
