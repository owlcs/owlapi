...
public enum Profiles implements HasIRI, KnownFactories, OWLProfile {
   ...
   * @return an OWLReasonerFactory if the class name represents an OWLReasonerFactory implementation
   * available on the classpath. Any exception raised by {@code Class.forName(factoryClassName)} is
  ...
  /**
   * @return collection of OWLReasonerFactory class names known to support the expressivity of this
   * profile. The factories can be instantiated through {@code instantiateFactory()} if the reasoner
   * classes are on the classpath. Note that this list is provided for information only, and might
   * be incorrect or incomplete due to changes in the reasoner implementations.<br> Should you know
   * of a reasoner not mentioned here, or find an error in the reported supported profiles, please
   * raise a bug about it.
   */
