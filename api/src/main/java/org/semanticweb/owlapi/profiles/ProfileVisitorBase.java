...
public class ProfileVisitorBase extends OWLOntologyWalkerVisitor {
    private static final OWLClassExpressionVisitorEx<Boolean> superClassExpressionChecker =
        new OWLClassExpressionVisitorExAdapter<Boolean>(Boolean.FALSE) {
            ...
            public Boolean visit(OWLObjectIntersectionOf ce) {
                return Boolean.valueOf(ce.getOperandsAsList().stream()
                    .noneMatch(e -> e.accept(this) == Boolean.FALSE));
            }
        ...
        new OWLClassExpressionVisitorExAdapter<Boolean>(Boolean.FALSE) {
            ...
            public Boolean visit(OWLObjectIntersectionOf ce) {
                return Boolean.valueOf(
                    ce.getOperandsAsList().stream().allMatch(op -> isOWL2RLSubClassExpression(op)));
            }
    ...
    static final OWLClassExpressionVisitorEx<Boolean> superClassRLExpressionChecker =
        new OWLClassExpressionVisitorExAdapter<Boolean>(Boolean.FALSE) {
            ...
            public Boolean visit(OWLObjectIntersectionOf ce) {
                return Boolean.valueOf(
                    ce.getOperandsAsList().stream().allMatch(e -> e.accept(this).booleanValue()));
            }
    ...
    protected static final Set<IRI> ALLOWED_RL_DATATYPES =
        OWL2Datatype.RL_DATATYPES.stream().map(HasIRI::getIRI).collect(Collectors.toSet());
    protected static final Set<IRI> ALLOWED_EL_DATATYPES =
        OWL2Datatype.EL_DATATYPES.stream().map(HasIRI::getIRI).collect(Collectors.toSet());
    ...
    protected OWLObjectPropertyManager getPropertyManager() {
        if (propertyManager == null) {
            propertyManager = new OWLObjectPropertyManager(
                getCurrentOntology().getOWLOntologyManager(), getCurrentOntology());
        }
    ...
    protected void insufficientOperands(OWLNaryDataRange node) {
        if (node.getOperands().size() < 2) {
            violations.add(new InsufficientOperands(getCurrentOntology(), getCurrentAxiom(), node));
        }
    }
    ...
    protected boolean rangePresent(OWLClassExpression imposedRange,
        ...
        return getCurrentOntology().getImportsClosure().stream()
            .flatMap(o -> o.getObjectPropertyRangeAxioms(lastProperty).stream())
    ...
    protected void ontologyIRINotAbsolute(OWLOntologyID id) {
        if (id.getOntologyIRI().isPresent()) {
            if (!id.getOntologyIRI().get().isAbsolute()) {
                violations.add(new OntologyIRINotAbsolute(getCurrentOntology(), id));
            }
        }
    }
    ...
    protected void versionNotAbsolute(OWLOntologyID id) {
        if (id.getVersionIRI().isPresent()) {
            if (!id.getVersionIRI().get().isAbsolute()) {
                violations.add(new OntologyVersionIRINotAbsolute(getCurrentOntology(), id));
            }
        }
    }
    ...
    private void getDatatypesInSignature(Set<OWLDatatype> datatypes, OWLObject obj,
        ...
        obj.getDatatypesInSignature().stream().filter(datatypes::add)
