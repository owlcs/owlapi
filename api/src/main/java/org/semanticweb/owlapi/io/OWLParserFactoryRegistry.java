...
/**
 * The {@code OWLParserFactoryRegistry} provides a central point for the registration of parser
 * factories that create parsers to parse OWL ontologies. The registry is typically used by at least
 * one type of ontology factory for loading ontologies whose concrete representations are contained
...
public class OWLParserFactoryRegistry extends AbstractServiceLoader<Set<OWLOntologyFormatFactory>, OWLParserFactory>
{
    ...
    {
        ArrayList<OWLParserFactory> all = new ArrayList<OWLParserFactory>(this.getAll());
        
        ArrayList<OWLParserFactory> list = new ArrayList<OWLParserFactory>(all.size());
        
        // HACK: The execution strategy used by OWLAPI, where all parsers are tried
        // and the first to succeed wins, is broken by the SPI random access, so need
        // to at least make sure that the RDF/XML parser comes before the OWLXML parser,
        // as they are both the same format and some RDF/XML documents seem to be detected
        // as syntactically valid OWLXML
        for(OWLParserFactory nextFactory : all)
        {
            for(OWLOntologyFormatFactory nextSupportedFormat : nextFactory.getSupportedFormats())
            {
                if(nextSupportedFormat.handlesMimeType(RDFFormat.RDFXML.getDefaultMIMEType()))
                {
                    list.add(nextFactory);
                }
                else if(nextSupportedFormat.handlesMimeType(RDFFormat.TURTLE.getDefaultMIMEType()))
                {
                    list.add(nextFactory);
                }
                // Manchester syntax is being parsed validly by the OBO parser, so make sure it
                // comes first
                else if(nextSupportedFormat.getKey().equals("Manchester OWL Syntax"))
                {
                    list.add(nextFactory);
                }
            }
        ...
        for(OWLParserFactory nextFactory : all)
        {
            ...
            {
                if(!nextSupportedFormat.handlesMimeType(RDFFormat.RDFXML.getDefaultMIMEType())
                        && !nextSupportedFormat.handlesMimeType(RDFFormat.TURTLE.getDefaultMIMEType())
                        && !(nextSupportedFormat.getKey().equals("Manchester OWL Syntax")))
                {
                    list.add(nextFactory);
                }
            }
     ...
     * @return An ontology parser factory matching the given mime type or null if none were found in
     *         the registry.
     */
     ...
     *            The {@link OWLOntologyFormatFactory} for the desired parser factory.
     * @return An ontology parser factory matching the given format or null if none were found in
     *         the registry.
     */
