...
public interface Node<E extends OWLObject> extends Iterable<E> {
   ...
   * owl:Thing}. <br> {@code true} if this node is an {@code OWLObjectProperty} node and it contains
   * {@code owl:topObjectProperty}. <br> {@code true} if this node is an {@code OWLDataProperty}
   * node and it contains {@code owl:topDataProperty}. <br> {@code false} if none of the above.
   */
  boolean isTopNode();
   ...
   * return set.
   * @return The set of entities that are contained in this node minus the specified entity, {@code
   * e}. If {@code e} is not contained within this node then the full set of entities returned is
   * the same as that returned by {@link #getEntities()}
   */
   ...
   *
   * @return The set of entities contained within this node minus the top entity. If this node does
   * not contain the top entity then the set of entities returned is the same as that returned by
   * {@link #getEntities()}.
   */
