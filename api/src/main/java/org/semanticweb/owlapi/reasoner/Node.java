 ...
 * hierarchy (directed acyclic graph - DAG) of {@code Nodes}. Each node
 ...
 * In a class hierarchy, the nodes contain {@code OWLClass} objects. The
 * top node contains {@code owl:Thing} (and any other named classes that
 * are equivalent to {@code owl:Thing}). The bottom node contains
 * {@code owl:Nothing} (and any other named classes that are equivalent to
 * {@code owl:Nothing} - these classes are <i>unsatisfiable</i> classes). <br>
 * In an object property hierarchy, the nodes contain
 * {@code OWLObjectProperty} objects. The top node contains
 ...
 * objects. The top node contains {@code owl:topDataProperty} (and any
 ...
 * {@code owl:bottomDataProperty}). <h4>Class Hierarchy Example</h4> The
...
public interface Node<E extends OWLObject> extends Iterable<E> {
     ...
     *         and it contains {@code owl:Thing}. <br>
     *         {@code true} if this node is an
     ...
     *         node and it contains {@code owl:topDataProperty}. <br>
     ...
     * 
     * @return {@code true} if this node is an {@code OWLClass} node
     *         and it contains {@code owl:Nothing}. <br>
     *         {@code true} if this node is an
     *         {@code OWLObjectProperty} node and it contains
     *         {@code owl:bottomObjectProperty}. <br>
     *         {@code true} if this node is an {@code OWLDataProperty}
     *         node and it contains {@code owl:bottomDataProperty}. <br>
     *         {@code false} if none of the above. */
