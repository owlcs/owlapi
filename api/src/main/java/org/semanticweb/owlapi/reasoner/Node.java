 ...
 * <code>Nodes</code>. Each node contains entities that are equivalent to each
 ...
 * <code>OWLClass</code> objects. The top node contains <code>owl:Thing</code>
 * (and any other named classes that are equivalent to <code>owl:Thing</code>).
 * The bottom node contains <code>owl:Nothing</code> (and any other named
 * classes that are equivalent to <code>owl:Nothing</code> - these classes are
 * <i>unsatisfiable</i> classes). </p> In an object property hierarchy, the
 * nodes contain <code>OWLObjectProperty</code> objects. The top node contains
 ...
 * properties that are equivalent to <code>owl:bottomObjectProperty</code>).
 * </p> In a data property hierarchy, the nodes contain
 * <code>OWLDataProperty</code> objects. The top node contains
 * <code>owl:topDataProperty</code> (and any other data properties that are
 * equivalent to <code>owl:topDataProperty</code>). The bottom node contains
 * <code>owl:bottomDataProperty</code> (and any other data properties that are
 * equivalent to <code>owl:bottomDataProperty</code>). <h4>Class Hierarchy
 * Example</h4> The figure below shows an example class hierarchy. Each box in
 * the hierarchy represents a <code>Node</code>. In this case the top node
 * contains <code>owl:Thing</code> and the bottom node contains
 * <code>owl:Nothing</code> because the nodes in the hierarchy are
 * <code>OWLClass</code> nodes. In this case, class <code>G</code> is equivalent
 * to <code>owl:Thing</code> so it appears as an entity in the top node.
 ...
 * <code>owl:Nothing</code>. </p> <div align=center"> <img
