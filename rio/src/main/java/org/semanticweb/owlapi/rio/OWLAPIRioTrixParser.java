 ...
 * <a href="http://www.w3.org/2004/03/trix/">TriX format</a>. Cloned from
 * org.openrdf.rio.trix.TrixParser
 * 
 * @author Arjohn Kampman Modified to install error handler by Simon Spero,
 *         because error handler.
 */
...
class OWLAPIRioTrixParser extends RDFParserBase {
    ...
    @Override
    public void parse(@Nullable InputStream in, @Nullable String baseURI) throws IOException, RDFParseException,
        RDFHandlerException {
        parse(new BOMInputStream(in, false));
    }

    @Override
    public void parse(@Nullable Reader reader, @Nullable String baseURI) throws IOException, RDFParseException,
        RDFHandlerException {
        parse(verifyNotNull(reader, "reader cannot be null"));
    }
    ...
    private class TriXSAXHandler extends SimpleSAXAdapter {
        ...
        @Nullable private Resource currentContext;
        ...
        @Override
        public void startTag(@Nullable String tagName, @Nullable Map<String, String> atts, @Nullable String text)
            throws SAXException {
            try {
                switch (verifyNotNull(tagName, "tagName cannot be null")) {
                    case URI_TAG:
                    ...
                    case BNODE_TAG:
                        ...
                        break;
                    case PLAIN_LITERAL_TAG:
                        String lang = verifyNotNull(atts, "attributes cannot be null").get(LANGUAGE_ATT);
                        valueList.add(createLiteral(text, lang, null));
                        break;
                    case TYPED_LITERAL_TAG:
                        String datatype = verifyNotNull(atts, "attributes cannot be null").get(DATATYPE_ATT);
                        if (datatype == null) {
                    ...
                    case TRIPLE_TAG:
                        if (parsingContext) {
                            try {
                                // First triple in a context, valueList can
                                // contain
                                // context information
                                if (valueList.size() > 1) {
        ...
        @Override
        public void endTag(@Nullable String tagName) throws SAXException {
            try {
                if (TRIPLE_TAG.equals(tagName)) {
                    reportStatement();
                } else if (CONTEXT_TAG.equals(tagName)) {
                    currentContext = null;
                }
            } catch (RDFParseException e) {
                throw new SAXException(e);
