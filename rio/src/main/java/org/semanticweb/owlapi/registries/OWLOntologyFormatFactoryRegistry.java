 ...
 */
public class OWLOntologyFormatFactoryRegistry extends
        AbstractServiceLoader<String, OWLOntologyFormatFactory> {
    ...
    /**
     * Returns the first OWLOntologyFormatFactory matching the mime type NOTE:
     * The order in which the services are loaded an examined is not
     * deterministic so this method may return different results if the
     * MIME-Type matches more than one OWLOntologyFormatFactory. However, if the
     * default MIME-Types are always unique, the correct factory will always be
     * chosen
     * 
     * @param mimeType
    ...
    public OWLOntologyFormatFactory getByMIMEType(String mimeType) {
        ...
        for (OWLOntologyFormatFactory nextFactory : getAll()) {
