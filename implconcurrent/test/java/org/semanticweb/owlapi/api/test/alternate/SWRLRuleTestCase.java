...
public class SWRLRuleTestCase extends AbstractAxiomsRoundTrippingTestCase {
	protected Set<? extends OWLAxiom> createAxioms() {
		...
		SWRLVariable varX = df.getSWRLVariable(IRI
				.create("http://www.owlapi#x"));
		SWRLVariable varY = df.getSWRLVariable(IRI
				.create("http://www.owlapi#y"));
		SWRLVariable varZ = df.getSWRLVariable(IRI
				.create("http://www.owlapi#z"));
		...
		SWRLVariable varQ = df.getSWRLVariable(IRI
				.create("http://www.owlapi#q"));
		SWRLVariable varR = df.getSWRLVariable(IRI
				.create("http://www.owlapi#r"));
		body.add(df
				.getSWRLDataPropertyAtom(getOWLDataProperty("d"), varX, varQ));
		OWLLiteral lit = df.getOWLLiteral(33);
		SWRLLiteralArgument litArg = df.getSWRLLiteralArgument(lit);
		body.add(df.getSWRLDataPropertyAtom(getOWLDataProperty("d"), varY,
				litArg));
		Set<SWRLAtom> head = new HashSet<SWRLAtom>();
		head.add(df.getSWRLClassAtom(getOWLClass("C"), varX));
		head.add(df.getSWRLObjectPropertyAtom(getOWLObjectProperty("p"), varY,
				varZ));
		...
		head.add(df.getSWRLBuiltInAtom(IRI
				.create("http://www.owlapi#myBuiltIn"), args));
