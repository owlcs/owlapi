	...
	public void testRoundTrip() {
		try {
			...
			manager.addAxiom(ontology, factory.getOWLAnnotationAssertionAxiom(
					factory.getRDFSLabel(), i, factory
							.getOWLLiteral(ANONYMOUS_INDIVIDUAL_ANNOTATION)));
			...
			manager.addAxiom(ontology, factory
					.getOWLObjectPropertyAssertionAxiom(factory
							.getOWLObjectProperty(IRI
									.create(NS + "#hasTopping")), i, j));
			File tmpFile = File.createTempFile("Test", ".owl");
			manager.saveOntology(ontology, new OWLXMLOntologyFormat(),
					new StreamDocumentTarget(new FileOutputStream(tmpFile)));
			OWLOntologyManager manager2 = ThreadSafeOWLManager.createOWLOntologyManager();
