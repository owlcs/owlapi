...
public class EquivalentClassesAxiomTestCase extends AbstractOWLAPITestCase {
	public void testContainsNamedClass() {
		...
		OWLClassExpression desc = getFactory().getOWLObjectSomeValuesFrom(
				propP, clsB);
		OWLClassExpression desc2 = getFactory().getOWLObjectSomeValuesFrom(
				propP, clsA);
		OWLEquivalentClassesAxiom ax = getFactory()
				.getOWLEquivalentClassesAxiom(clsA, desc);
		assertTrue(ax.containsNamedEquivalentClass());
		OWLEquivalentClassesAxiom ax2 = getFactory()
				.getOWLEquivalentClassesAxiom(desc, desc2);
	...
	public void testGetNamedClassesWithNothing() {
		...
		OWLClassExpression desc = getFactory().getOWLObjectSomeValuesFrom(
				propP, clsB);
		OWLEquivalentClassesAxiom ax = getFactory()
				.getOWLEquivalentClassesAxiom(getFactory().getOWLNothing(),
						desc);
		Set<OWLClass> clses = ax.getNamedClasses();
	...
	public void testGetNamedClassesWithThing() {
		...
		OWLClassExpression desc = getFactory().getOWLObjectSomeValuesFrom(
				propP, clsB);
		OWLEquivalentClassesAxiom ax = getFactory()
				.getOWLEquivalentClassesAxiom(getFactory().getOWLThing(), desc);
		Set<OWLClass> clses = ax.getNamedClasses();
		assertTrue(clses.isEmpty());
	...
	public void testSplit() {
		...
		clses.add(clsC);
		OWLEquivalentClassesAxiom ax = getFactory()
				.getOWLEquivalentClassesAxiom(clses);
		Set<OWLSubClassOfAxiom> scas = ax.asOWLSubClassOfAxioms();
		assertEquals(scas.size(), 6);
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsA, clsB)));
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsB, clsA)));
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsA, clsC)));
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsC, clsA)));
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsB, clsC)));
		assertTrue(scas
				.contains(getFactory().getOWLSubClassOfAxiom(clsC, clsB)));
	}
}
