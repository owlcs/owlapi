...
public class ProfileValidationTestCase extends TestCase {
	...
	private static final IRI TEST_CASE_IRI = IRI.create(TEST_NAMESPACE
			+ "TestCase");
	private static final IRI PROFILE_IDENTIFICATION_TEST_IRI = IRI
			.create(TEST_NAMESPACE + "ProfileIdentificationTest");
	private static final IRI PROFILE_IRI = IRI.create(TEST_NAMESPACE
			+ "profile");
	private static final IRI SPECIES_IRI = IRI.create(TEST_NAMESPACE
			+ "species");
	...
	private static final IRI RDF_XML_PREMISE_ONTOLOGY_IRI = IRI
			.create(TEST_NAMESPACE + "rdfXmlPremiseOntology");
	...
	private void checkProfile(OWLOntology ontology, OWLProfile profile,
		...
		if (report.isInProfile() != shouldBeInProfile) {
			if (shouldBeInProfile) {
				System.out.println("FAIL: " + ontology.getOntologyID()
						+ " should be in the " + profile.getName()
						+ " profile. Report: " + report);
			} else {
				System.out.println("FAIL: " + ontology.getOntologyID()
						+ " should not be in the " + profile.getName()
						+ " profile. Report: " + report);
			}
		}
	}
