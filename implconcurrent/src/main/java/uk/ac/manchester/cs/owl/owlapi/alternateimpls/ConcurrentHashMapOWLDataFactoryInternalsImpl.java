...
public class ConcurrentHashMapOWLDataFactoryInternalsImpl implements
	...
	private final Thread reaper = new Thread() {
		...
		private void clean(Map<IRI, WeakReference<? extends OWLEntity>> map) {
			if (map != null) {
				...
				if (l.size() > 0) {
					System.out
							.println("ConcurrentHashMap: removed " + l.size());
				}
			}
		}
	};

	public ConcurrentHashMapOWLDataFactoryInternalsImpl(OWLDataFactory f) {
		factory = f;
