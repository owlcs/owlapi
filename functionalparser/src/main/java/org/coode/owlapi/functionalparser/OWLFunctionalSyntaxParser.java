...
public class OWLFunctionalSyntaxParser implements OWLFunctionalSyntaxParserConstants {
    ...
    public URI getURI(String s) throws URISyntaxException {
        URI iri = string2URI.get(s);
        if(iri == null) {
            if(s.charAt(0) == '<') {
                iri = new URI(s.substring(1, s.length() - 1));
            }
            else {
                ...
                String fullURIString = prefix + s.substring(colonIndex + 1);
                iri = new URI(fullURIString);
            }
            string2URI.put(s, iri);
        }
        return iri;
    }
  ...
  final public IRI FullIRI() throws ParseException {
    ...
    t = jj_consume_token(FULLIRI);
        try {
            {if (true) return dataFactory.getIRI(getURI(t.image));}
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  final public OWLFacetRestriction DataRangeFacetRestriction() throws ParseException {
    ...
    iri = IRI();
    con = Literal();
        OWLFacet v = OWLFacet.getFacetByShortName(iri.toURI().getFragment());
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  final public SWRLIArgument IArg() throws ParseException {
    ...
    if (jj_2_128(2)) {
      ...
      jj_consume_token(INDIVIDUALVARIABLE);
      jj_consume_token(OPENPAR);
      iri = IRI();
      jj_consume_token(CLOSEPAR);
                                                              {if (true) return dataFactory.getSWRLIndividualVariable(iri);}
    } else {
      jj_consume_token(-1);
      throw new ParseException();
    }
  ...
  final public SWRLDArgument DArg() throws ParseException {
    ...
    if (jj_2_130(2)) {
      ...
      jj_consume_token(LITERALVARIABLE);
      jj_consume_token(OPENPAR);
      iri = IRI();
      jj_consume_token(CLOSEPAR);
                                                                                                                                 {if (true) return dataFactory.getSWRLLiteralVariable(iri);}
    } else {
      jj_consume_token(-1);
      throw new ParseException();
    }
  ...
  private boolean jj_3_131() {
    if (jj_scan_token(LITERALVARIABLE)) return true;
    if (jj_scan_token(OPENPAR)) return true;
    return false;
  }
  ...
  private boolean jj_3_129() {
    if (jj_scan_token(INDIVIDUALVARIABLE)) return true;
    if (jj_scan_token(OPENPAR)) return true;
    return false;
  }
