...
public class SyntacticLocalityEvaluator implements LocalityEvaluator {
  ...
  protected static boolean isTopOrBuiltInInfiniteDatatype(OWLDataRange dataRange) {
    if (dataRange.isOWLDatatype()) {
      OWLDatatype dataType = dataRange.asOWLDatatype();
      return dataType.isTopDatatype() || dataType.isBuiltIn() && !dataType.getBuiltInDatatype()
          .isFinite();
    } else {
      return false;
    }
  ...
  private static class BottomEquivalenceEvaluator implements OWLClassExpressionVisitor {
    ...
    public void visit(OWLDataAllValuesFrom ce) {
      switch (getLocality()) {
        ...
        case TOP_BOTTOM:
          ...
          break;
        case TOP_TOP:
          isBottomEquivalent = !getSignature().contains(ce.getProperty().asOWLDataProperty()) && !ce
              .getFiller().isTopDatatype();
          break;
        default:
          break;
      }
    ...
    public void visit(OWLDataExactCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isBottomEquivalent = ce.getCardinality() == 0 && !getSignature().contains(ce.getProperty()
              .asOWLDataProperty()) && isTopOrBuiltInDatatype(ce.getFiller())
              || ce.getCardinality() > 0
    ...
    public void visit(OWLDataMaxCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          ...
          isBottomEquivalent = zeroCard(ce, p) || oneCard(ce, p) || moreThanOneCard(ce, p);
          break;
        default:
          break;
      }
    ...
    protected boolean moreThanOneCard(OWLDataMaxCardinality ce, OWLDataProperty p) {
      return ce.getCardinality() > 1 && !getSignature().contains(p)
          && isTopOrBuiltInInfiniteDatatype(ce
          .getFiller());
    }

    protected boolean oneCard(OWLDataMaxCardinality ce, OWLDataProperty p) {
      return ce.getCardinality() == 1 && !getSignature().contains(p) && isTopOrBuiltInDatatype(
          ce.getFiller());
    }

    protected boolean zeroCard(OWLDataMaxCardinality ce, OWLDataProperty p) {
      return ce.getCardinality() == 0 && !getSignature().contains(p) && isTopOrBuiltInDatatype(
          ce.getFiller());
    }
    ...
    public void visit(OWLObjectComplementOf ce) {
      isBottomEquivalent = topEvaluator
          .isTopEquivalent(ce.getOperand(), getSignature(), getLocality());
    }
      ...
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isBottomEquivalent = ce.getCardinality() > 0 && (isBottomEquivalent(ce.getFiller())
              || !getSignature().contains(ce.getProperty().getNamedProperty()) && topEvaluator
              .isTopEquivalent(ce.getFiller(), getSignature(), getLocality()));
          break;
        default:
          break;
      }
    ...
    public void visit(OWLObjectMaxCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isBottomEquivalent = ce.getCardinality() > 0 && !getSignature().contains(ce.getProperty()
              .getNamedProperty()) && topEvaluator.isTopEquivalent(ce.getFiller(), getSignature(),
              getLocality());
          break;
        default:
          break;
    ...
    public void visit(OWLObjectUnionOf ce) {
      isBottomEquivalent = !ce.operands().anyMatch(d -> !isBottomEquivalent(d));
    }
  ...
  private static class TopEquivalenceEvaluator implements OWLClassExpressionVisitor {
    ...
    public void visit(OWLDataAllValuesFrom ce) {
      switch (getLocality()) {
        ...
        case TOP_BOTTOM:
          isTopEquivalent =
              !signature.contains(ce.getProperty().asOWLDataProperty()) || ce.getFiller()
                  .isTopDatatype();
          break;
        case TOP_TOP:
    ...
    public void visit(OWLDataMinCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isTopEquivalent =
              ce.getCardinality() == 0 || ce.getCardinality() == 1 && !signature.contains(ce
                  .getProperty().asOWLDataProperty()) && isTopOrBuiltInDatatype(ce.getFiller())
                  || ce
    ...
    public void visit(OWLObjectAllValuesFrom ce) {
      switch (getLocality()) {
        ...
        case TOP_BOTTOM:
          isTopEquivalent =
              !signature.contains(ce.getProperty().getNamedProperty()) || isTopEquivalent(ce
                  .getFiller());
          break;
        case TOP_TOP:
    ...
    @Override
    public void visit(OWLObjectComplementOf ce) {
      isTopEquivalent = bottomEvaluator
          .isBottomEquivalent(ce.getOperand(), getSignature(), getLocality());
    }
    ...
    public void visit(OWLObjectExactCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isTopEquivalent =
              ce.getCardinality() == 0 && bottomEvaluator.isBottomEquivalent(ce.getFiller(),
    ...
    public void visit(OWLObjectMinCardinality ce) {
      switch (getLocality()) {
        ...
        case TOP_TOP:
          isTopEquivalent = ce.getCardinality() == 0 || !signature.contains(ce.getProperty()
              .getNamedProperty()) && isTopEquivalent(ce.getFiller());
          break;
        default:
          break;
      }
  ...
  private class AxiomLocalityVisitor implements OWLAxiomVisitor {
    ...
    public void visit(OWLDataPropertyDomainAxiom axiom) {
      switch (localityCls) {
        ...
        case TOP_BOTTOM:
          isLocal =
              !getSignature().contains(axiom.getProperty().asOWLDataProperty()) || topEvaluator
    ...
    public void visit(OWLDataPropertyRangeAxiom axiom) {
      switch (localityCls) {
        ...
        case TOP_BOTTOM:
          isLocal =
              !getSignature().contains(axiom.getProperty().asOWLDataProperty()) || axiom.getRange()
                  .isTopDatatype();
          break;
        case TOP_TOP:
    ...
    protected boolean checkIfLocalProperties(
      ...
      if (size > 1) {
        ...
        for (OWLPropertyExpression dpe : disjs) {
          if (dpe.isOWLDataProperty() && getSignature().contains(dpe.asOWLDataProperty()) || dpe
              .isOWLObjectProperty() && getSignature().contains(dpe.asOWLObjectProperty())) {
    ...
    public void visit(OWLInverseObjectPropertiesAxiom axiom) {
      isLocal =
          !getSignature().contains(axiom.getFirstProperty().getNamedProperty()) && !getSignature()
              .contains(
    ...
    public void visit(OWLSubPropertyChainOfAxiom axiom) {
      switch (localityCls) {
        ...
        case TOP_BOTTOM:
          ...
          // bot-equiv
          isLocal = axiom.getPropertyChain().stream().anyMatch(ope -> !getSignature().contains(ope
              .getNamedProperty()));
          break;
        case TOP_TOP:
