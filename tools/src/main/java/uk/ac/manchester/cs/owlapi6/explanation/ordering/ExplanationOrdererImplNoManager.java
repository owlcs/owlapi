...
package uk.ac.manchester.cs.owlapi6.explanation.ordering;
...
import org.semanticweb.owlapi6.model.OWLPropertyAxiom;
...
public class ExplanationOrdererImplNoManager implements ExplanationOrderer {
    ...
    @Override
    public ExplanationTree getOrderedExplanation(OWLAxiom entailment, Set<OWLAxiom> axiomValues) {
        currentExplanation = new HashSet<>(axiomValues);
        buildIndices();
    ...
    private static class SeedExtractor implements OWLAxiomVisitor {
        ...
        public void visit(OWLDisjointClassesAxiom axiom) {
            axiom.classExpressions().filter(IsAnonymous::isNamed).forEach(this::addSourceOrTarget);
        }

        protected void addSourceOrTarget(OWLClassExpression ce) {
            if (source == null) {
    ...
    class AxiomMapBuilder implements OWLAxiomVisitor {
        ...
        public void visit(OWLSameIndividualAxiom axiom) {
            axiom.individuals().filter(IsAnonymous::isNamed).forEach(ind -> {
                getAxiomsForLHS(ind.asOWLNamedIndividual()).add(axiom);
                indexAxiomsByRHSEntities(ind, axiom);
            });
        }
        ...
        public void visit(OWLInverseObjectPropertiesAxiom axiom) {
            if (axiom.getFirstProperty().isNamed()) {
