...
package org.semanticweb.owlapi6.modularity;
...
import com.clarkparsia.owlapi6.modularity.locality.LocalityClass;
import com.clarkparsia.owlapi6.modularity.locality.SyntacticLocalityEvaluator;
...
public class SyntacticLocalityModuleExtractor implements OntologySegmenter {
    ...
    public SyntacticLocalityModuleExtractor(OWLOntologyManager man, Stream<OWLAxiom> axs,
        ...
        this(man, man.getOntologyConfigurator(), axs, moduleType, false);
    }
    ...
    public SyntacticLocalityModuleExtractor(OWLOntologyManager man, Stream<OWLAxiom> axs,
        ...
        this(man, man.getOntologyConfigurator(), axs, moduleType, excludeAssertions);
    }
     ...
     * @param excludeAssertions true if assertions should be excluded
     */
    public SyntacticLocalityModuleExtractor(OWLOntologyManager man, OntologyConfigurator config,
        Stream<OWLAxiom> axs, ModuleType moduleType, boolean excludeAssertions) {
        this.config = config;
    ...
    public SyntacticLocalityModuleExtractor(OWLOntologyManager man, OWLOntology ont,
        ModuleType moduleType) {
        this(man, ont.getOntologyConfigurator(), asAxiomSet(ont), moduleType, false);
    }
        ...
        while (change) {
            ...
            for (OWLAxiom ax : q2) {
                if (!sle.isLocal(ax, signature)) {
                    LOGGER.info("      Non-local axiom:   {}", ax);
                    mod.add(ax);
                    q2remove.add(ax);
                    int oldSize = signature.size();
