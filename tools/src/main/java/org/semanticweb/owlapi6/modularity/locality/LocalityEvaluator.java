...
package org.semanticweb.owlapi6.modularity.locality;
...
/*
 * Following com.clarkparsia.owlapi.modularity.locality.LocalityEvaluator and
...
public interface LocalityEvaluator {
    ...
    /**
     * Tests whether the given axiom is local with respect to the given signature. For efficient
     * computation, the given collection should run {@link Collection#contains(Object)} in O(1).
     *
     * @param axiom     the axiom to test
     * @param signature the signature to test against
     * @return <code>true</code> if the axiom is local w.r.t. the signature; <code>false</code>
     *         otherwise
     */
     ...
     *         otherwise
     */
    default boolean isLocal(OWLAxiom axiom, Stream<OWLEntity> signature) {
        return isLocal(axiom, signature.collect(Collectors.toSet()));
    }
