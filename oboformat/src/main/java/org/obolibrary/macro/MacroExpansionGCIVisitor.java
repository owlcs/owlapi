...
public class MacroExpansionGCIVisitor {
   ...
   * @param preserveAnnotationsWhenExpanding true if annotations should be preserved when expanding
   */
        ...
        try {
          visitor.getTool().parseManchesterExpressionFrames(expandTo).forEach(axp -> {
            ...
            if (shouldPreserveAnnotationsWhenExpanding()) {
              axiom = axiom
                  .getAnnotatedAxiom(visitor.getAnnotationsWithOptionalExpansionMarker(ax));
            }
            newAxioms.add(axiom);
