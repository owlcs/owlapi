...
public class TypeFunctionalPropertyHandler extends BuiltInTypeHandler {
    ...
    public void handleTriple(IRI subject, IRI predicate, IRI object)
        ...
        if (getConsumer().isObjectProperty(subject)) {
            addAxiom(getDataFactory().getOWLFunctionalObjectPropertyAxiom(
                    translateObjectProperty(subject), getPendingAnnotations()));
            consumeTriple(subject, predicate, object);
        }
        if (getConsumer().isDataProperty(subject)) {
            addAxiom(getDataFactory().getOWLFunctionalDataPropertyAxiom(
                    translateDataProperty(subject), getPendingAnnotations()));
            consumeTriple(subject, predicate, object);
        }
    }
}
