...
import static org.semanticweb.owlapi.rdf.rdfxml.parser.HandlerFunction.*;
...
enum AbstractTriplePH implements ResourceTripleHandler, TriplePredicateHandler, LiteralTripleHandler {
    FirstLiteralHandler(
    ...
    AnnotationResourceTripleHandler(
        null,
    ...
    ObjectPropertyAssertionHandler(
        null,
    ...
    DatatypeComplementOfHandler(
        ...
        NO_OP_IRI,
        (c, s, p, o) -> c.canHandleDatatypeComplementOfStreaming(s, o),
        NO_OP_IRI),
    OnClassHandler(OWL_ON_CLASS, NO_OP_IRI, (c, s, p, o) -> c.canHandleOnClassStreamng(o), NO_OP_IRI),
    OnDataRangeHandler(OWL_ON_DATA_RANGE, NO_OP_IRI, (c, s, p, o) -> c.canHandleOnDataStreaming(o), NO_OP_IRI),
    OnPropertyHandler(
        OWL_ON_PROPERTY,
    ...
    /**
     * General literal triples - i.e. triples which have a predicate that is not
     * a built in IRI. Annotation properties get precedence over data
    ...
    public static List<LiteralTripleHandler> getLiteralTripleHandlers() {
        return Arrays.asList(DataPropertyAssertionHandler, FirstLiteralHandler, AnnotationLiteralHandler);
    }

    public static Map<IRI, TriplePredicateHandler> getPredicateHandlers() {
        return Stream.of(DifferentFromHandler, DisjointUnionHandler, DisjointWithHandler, EquivalentClassHandler,
            EquivalentPropertyHandler, PropertyDomainHandler, PropertyRangeHandler, SameAsHandler, SubClassOfHandler,
            SubPropertyOfHandler, TypeHandler, ImportsHandler, IntersectionOfHandler, UnionOfHandler,
            ComplementOfHandler, OneOfHandler, HasValueHandler, SomeValuesFromHandler, AllValuesFromHandler,
            RestHandler, FirstResourceHandler, DeclaredAsHandler, HasKeyHandler, VersionIRIHandler, PropertyChainAH,
            AnnotatedSourceHandler, AnnotatedPropertyHandler, AnnotatedTargetHandler, PropertyDisjointWithHandler,
            InverseOfHandler, OnPropertyHandler, OnClassHandler, OnDataRangeHandler, DatatypeComplementOfHandler)
    ...
    AbstractTriplePH(HasIRI i, HandlerFunction<IRI> canHandle, HandlerFunction<IRI> canHandleStreaming,
        HandlerFunction<IRI> handle) {
        this(i, canHandle, canHandleStreaming, handle, NO_OP_LIT, NO_OP_LIT, NO_OP_LIT);
    }

    AbstractTriplePH(HandlerFunction<OWLLiteral> canHandle, HandlerFunction<OWLLiteral> canHandleStreaming,
        HandlerFunction<OWLLiteral> handle) {
        this(null, NO_OP_IRI, NO_OP_IRI, NO_OP_IRI, canHandle, canHandleStreaming, handle);
    }

    AbstractTriplePH(HasIRI i, HandlerFunction<IRI> canHandle, HandlerFunction<IRI> canHandleStreaming,
        HandlerFunction<IRI> handle, HandlerFunction<OWLLiteral> canHandleLit,
        HandlerFunction<OWLLiteral> canHandleStreamingLit, HandlerFunction<OWLLiteral> handleLit) {
        predicateIRI = i;
    ...
    @Override
    public final boolean canHandleStreaming(OWLRDFConsumer c, IRI subject, IRI predicate, IRI object) {
    ...
    @Override
    public final boolean canHandle(OWLRDFConsumer c, IRI subject, IRI predicate, OWLLiteral object) {
        return canHandleLit.handle(c, subject, predicate, object);
    }

    @Override
    public final boolean canHandleStreaming(OWLRDFConsumer c, IRI subject, IRI predicate, OWLLiteral object) {
        return canHandleStreamingLit.handle(c, subject, predicate, object);
    }

    @Override
    public final boolean handleTriple(OWLRDFConsumer c, IRI subject, IRI predicate, OWLLiteral object) {
