...
public abstract class RDFRendererBase {
    ...
    private void findPuns() {
        ...
        if(ontologyIRI.isPresent()) {
            punCounts.put(ontologyIRI.get(),1);
        }
        ...
        incrementPunCounts(ontology.getAnnotationPropertiesInSignature(Imports.EXCLUDED));
    }
    ...
    private void renderUntypedIRIAnnotationAssertions() throws IOException {
        ...
        for (OWLAnnotationAssertionAxiom ax : ontology
            ...
            if (subject instanceof IRI) {
                IRI iri = (IRI) subject;
                if (isPunned(iri) || !ontology.containsEntityInSignature(iri, EXCLUDED)) {
    ...
    private void addAnnotationsToOntologyHeader(
            @Nonnull RDFResource ontologyHeaderNode) {
        Optional<IRI> ontologyIRI = ontology.getOntologyID().getOntologyIRI();
        if(ontologyIRI.isPresent() && isPunned(ontologyIRI.get())) {
            return;
        }
