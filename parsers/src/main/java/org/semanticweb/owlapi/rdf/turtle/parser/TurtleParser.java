...
{if ("" != null) return iri;}
  ...
  final public IRI parseLoneNS() throws ParseException {Token t;
    t = jj_consume_token(PNAME_NS);
{if ("" != null) return getIRIFromQName(t.image);}
    throw new Error("Missing return statement in function");
}
...
{if ("" != null) return getNextBlankNode(t.image);}
    throw new Error("Missing return statement in function");
}
...
{if ("" != null) return EscapeUtils.unescapeString(rawString);}
    throw new Error("Missing return statement in function");
}
 ...
 {
    if (jj_scan_token(LONG_SINGLESTRING)) return true;
    return false;
  }
 ...
 {
    if (jj_scan_token(SINGLESTRING)) return true;
    return false;
  }
