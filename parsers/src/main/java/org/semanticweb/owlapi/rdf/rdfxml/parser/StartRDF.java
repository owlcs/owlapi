...
abstract class AbstractState {
  ...
  void propertyAttributes(String subjectIRI, Attributes atts,
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (!XMLNS.equals(nsIRI) && !XMLLANG.equals(localName) && !(RDFNS.equals(nsIRI) && (
          ATTR_ID.equals(
              localName) || ATTR_NODE_ID.equals(localName) || ATTR_ABOUT.equals(localName)
              || ELT_TYPE.equals(
...
class NodeElement extends AbstractState implements State {
  ...
  public void startElement(String namespaceIRI, String localName, String qName, Attributes atts) {
    ...
    parser
        .verify(atts.getIndex(RDFNS, ATTR_ABOUT_EACH_PREFIX) != -1, ABOUT_EACH_PREFIX_UNSUPPORTED);
  ...
  public void startElement(String namespaceIRI, String localName, String qName, Attributes atts) {
    if (depth == 0) {
      ...
      if (!localName.equals(qName) && declaredNamespaces.add(namespaceIRI)) {
        m_content.append(" xmlns:").append(qName.substring(0, qName.indexOf(':'))).append("=\"")
            .append(
                namespaceIRI).append('"');
      }
      int length = atts.getLength();
...
class PropertyElementList extends AbstractState implements State {
  ...
  public void startElement(String namespaceIRI, String localName, String qName, Attributes atts)
    ...
    // parseType=Literal
    if (PARSE_TYPE_LITERAL.equals(parseType) || RDF_XMLLITERAL
        .equals(atts.getValue(RDFNS, ATTR_DATATYPE))) {
...
class ReifiedStatementBag extends ReificationManager {
  ...
  protected String getReificationID(@Nullable String reificationID, RDFParser parser) {
    ...
    parser
        .statementWithResourceValue(iri, RDFNS + '_' + elements.getAndIncrement(), resultIRI, null);
    return resultIRI;
  }
}
