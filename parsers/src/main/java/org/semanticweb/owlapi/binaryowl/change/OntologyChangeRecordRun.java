...
public class OntologyChangeRecordRun {
    ...
    public OntologyChangeRecordRun(DataInput dataInputStream, OWLDataFactory dataFactory) throws IOException, BinaryOWLParseException {
    ...
    public List<OWLOntologyChangeData> getInfoList() {
    ...
    public void write(DataOutputStream dos) throws IOException {
        ...
        for(OWLOntologyChangeData info : records) {
            OntologyChangeRecordInfoType.write(info, LookupTable.emptyLookupTable(), dos);
        }
    }
    
    private void read(DataInput dataInput, OWLDataFactory dataFactory) throws IOException, BinaryOWLParseException {
        BinaryOWLOntologyID idSerializer = new BinaryOWLOntologyID(dataInput, dataFactory);
        ontologyID = idSerializer.getOntologyID();
        int recordCount = dataInput.readInt();
        records = new ArrayList<OWLOntologyChangeData>(recordCount + 1);
        for(int i = 0; i < recordCount; i++) {
            OWLOntologyChangeData info = OntologyChangeRecordInfoType.read(dataInput, LookupTable.emptyLookupTable(), dataFactory);
            records.add(info);
        }
    }
}
