...
public class Owl2OboTestCase extends OboFormatTestBasics {
    ...
    public void testOwl2OboAltIdClass() throws Exception {
        ...
        assertEquals(4,
            roundTripped.annotationAssertionAxioms(classB.getIRI()).count());
        // for the comment
        Optional<OWLLiteral> comment = findComment(classB.getIRI(),
            roundTripped);
        assertTrue(comment.isPresent());
        assertEquals("Comment", comment.get().getLiteral());
    }

    protected Optional<OWLLiteral> findComment(IRI i,
        OWLOntology roundTripped) {
        return roundTripped.annotationAssertionAxioms(i)
            .filter(ax -> ax.getProperty().isComment())
            .map(ax -> ax.getValue().asLiteral()).filter(l -> l.isPresent())
    ...
    public void testOwl2OboProperty() throws Exception {
        ...
        assertEquals(4,
            roundTripped.annotationAssertionAxioms(p2.getIRI()).count());
        // for the comment
    ...
    private static void addLabelAndId(OWLNamedObject obj, String label,
        String id, OWLOntology o) {
    ...
    private static void setAltId(OWLNamedObject obj, OWLOntology o) {
        ...
        addAnnotation(obj,
            f.getOWLAnnotationProperty(
                Obo2OWLVocabulary.IRI_IAO_0100001.getIRI()),
    ...
    private static void addAnnotation(OWLNamedObject obj,
        OWLAnnotationProperty p, OWLAnnotationValue v, OWLOntology ont) {
