...
public class RioRendererTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        // limit the storers to known Rio OntologyStorers to minimise false negative results
        ...
        testManager =
                OWLOntologyManagerFactoryRegistry.createOWLOntologyManager(
                        OWLOntologyManagerFactoryRegistry.getOWLDataFactory(), storerRegistry, parserRegistry);
    ...
    public void testRenderEmptyStatementCollector() throws IOException
    {
        RioRenderer testRenderer =
                new RioRenderer(testOntologyEmpty, testHandlerStatementCollector, testFormatNull);
    ...
    {
        RioRenderer testRenderer = new RioRenderer(testOntologyEmpty, testRdfXmlRioWriter, testFormatNull);
    ...
    {
        RioRenderer testRenderer = new RioRenderer(testOntologyEmpty, testTurtleRioWriter, testFormatNull);
    ...
    {
        RioRenderer testRenderer =
                new RioRenderer(testOntologyEmpty, testNTriplesRioWriter, testFormatNull);
    ...
    {
        RioRenderer testRenderer =
                new RioRenderer(testOntologyKoala, testHandlerStatementCollector, testFormatNull);
        ...
        HashSet<Statement> resultStatements = new HashSet<Statement>(testHandlerStatementCollector.getStatements());
     ...
     */
    @Test
    public void testRenderKoalaRdfXmlWriter() throws IOException, RDFParseException, RDFHandlerException
    {
        RioRenderer testRenderer = new RioRenderer(testOntologyKoala, testRdfXmlRioWriter, testFormatNull);
        ...
        // actual length depends on the length of dynamically assigned blank node identifiers, so we
        ...
        // NOTE: The base xmlns: does not get counted as a namespace by the Rio RDFXML parser, which
        // is why it counts to 5, compared to direct StatementCollector result and the turtle result
     ...
     */
    @Test
    public void testRenderKoalaTurtleWriter() throws IOException, RDFParseException, RDFHandlerException
    {
        RioRenderer testRenderer = new RioRenderer(testOntologyKoala, testTurtleRioWriter, testFormatNull);
        ...
        // actual length depends on the length of dynamically assigned blank node identifiers, so we
        // only test a minimum length and a maximum length
        assertTrue("result.length()=" + result.length() + " was not inside the expected bounds", result.length() > 8500);
        assertTrue("result.length()=" + result.length() + " was not inside the expected bounds",
                result.length() < 9500);
     ...
     */
    @Test
    public void testRenderKoalaNTriplesWriter() throws IOException, RDFParseException, RDFHandlerException
    {
        RioRenderer testRenderer =
                new RioRenderer(testOntologyKoala, testNTriplesRioWriter, testFormatNull);
        ...
        // NTriples does not contain namespaces, so we will not find any when parsing the document
