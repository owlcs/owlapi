...
public class PrimerTestCase extends TestBase {
    ...
    @Nonnull OWLOntology func = loadOntologyFromString(FUNCTIONAL, IRI.create("urn:primer#functional"),
    ...
    public void shouldManchBeEquivalent() throws OWLOntologyCreationException {
        ...
        OWLClass x = df.getOWLClass(IRI.create("http://example.com/owl/families/X"));
        Set<OWLClassAxiom> axioms = manch.getAxioms(x, EXCLUDED);
        manch.getOWLOntologyManager().removeAxioms(manch, axioms);
        OWLClass female = df.getOWLClass(IRI.create("http://example.com/owl/families/Female"));
        OWLClassExpression oneOf = df.getOWLObjectOneOf(df.getOWLNamedIndividual(IRI.create(
            "http://example.com/owl/families/Bill")), df.getOWLNamedIndividual(IRI.create(
                "http://example.com/owl/families/Mary")), df.getOWLNamedIndividual(IRI.create(
                    "http://example.com/owl/families/Meg")));
        OWLClass parent = df.getOWLClass(IRI.create("http://example.com/owl/families/Parent"));
        OWLObjectProperty hasChild = df.getOWLObjectProperty(IRI.create("http://example.com/owl/families/hasChild"));
        ...
        manch.getOWLOntologyManager().addAxiom(manch, df.getOWLSubClassOfAxiom(df.getOWLObjectIntersectionOf(female,
            oneOf), superClass));
        OWLOntology replacement = m.createOntology(manch.getAxioms(), manch.getOntologyID().getOntologyIRI().get());
    ...
    public void shouldTURTLEBeEquivalent() {
        ...
        OWLObjectProperty hasParent = df.getOWLObjectProperty(IRI.create("http://example.com/owl/families/hasParent"));
        turt.getOWLOntologyManager().removeAxiom(turt, df.getOWLInverseObjectPropertiesAxiom(df.getOWLObjectInverseOf(
            hasParent), hasParent));
        equal(func, turt);
    }

    @Nonnull private static final String RDFXML = "<!DOCTYPE rdf:RDF [\n"
        ...
        + " </rdf:RDF>";
    @Nonnull private static final String OWLXML = "<!DOCTYPE Ontology [\n"
        + "        <!ENTITY xsd \"http://www.w3.org/2001/XMLSchema#\" >\n"
        + "        <!ENTITY rdfs \"http://www.w3.org/2000/01/rdf-schema#\" >\n" + "        ]>\n" + '\n'
        ...
        + '\n' + "</Ontology>";
    @Nonnull private static final String FUNCTIONAL = " Prefix(:=<http://example.com/owl/families/>)\n"
    ...
    @Nonnull private static final String MANCHESTER = "Prefix: : <http://example.com/owl/families/>\n"
        + "Prefix: xsd: <http://www.w3.org/2001/XMLSchema#>\n" + "Prefix: owl: <http://www.w3.org/2002/07/owl#>\n"
        + "Prefix: otherOnt: <http://example.org/otherOntologies/families/>\n"
        + "Ontology: <http://example.com/owl/families>\n"
        ...
        + "SameIndividual: Mary, otherOnt:MaryBrown";
    @Nonnull private static final String TURTLE = "@prefix : <http://example.com/owl/families/> .\n"
        + "@prefix otherOnt: <http://example.org/otherOntologies/families/> .\n"
        + "@prefix owl: <http://www.w3.org/2002/07/owl#> .\n"
