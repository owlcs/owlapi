...
public class AnnotatetAnnotationsTestCase extends AbstractOWLAPITestCase {
    ...
    public void shouldRoundtripMultipleNestedAnnotationsdebug() throws OWLOntologyCreationException {
        ...
        OWLOntology ont = getManager().loadOntologyFromOntologyDocument(new StringDocumentSource(input));
    ...
    public void shouldLoadAnnotatedannotationsCorrectly()
        ...
        String input = "<?xml version=\"1.0\"?>\n"
            ...
            + "        <owl:annotatedProperty rdf:resource=\"http://www.geneontology.org/formats/oboInOwl#hasDbXref\"/>\n"
            + "    </owl:Axiom>\n" + "</rdf:RDF>";
        OWLOntology testcase = getManager().loadOntologyFromOntologyDocument(new StringDocumentSource(input));
        int before = testcase.getAnnotationAssertionAxioms(subject).size();
        StringDocumentTarget target = new StringDocumentTarget();
        getManager().saveOntology(testcase, target);
        getManager().removeOntology(testcase);
        OWLOntology result = getManager().loadOntologyFromOntologyDocument(new StringDocumentSource(target));
    ...
    public void
        ...
        OWLAnnotationProperty p = AnnotationProperty(IRI.create("urn:t:p"));
        OWLAnnotationSubject i = IRI.create("urn:t:i");
        ...
        assertNotEquals(0, ax1.compareTo(ax2));
        Set<OWLAnnotationAssertionAxiom> set = new TreeSet<OWLAnnotationAssertionAxiom>();
        set.add(ax1);
        set.add(ax2);
