...
public class OWLOntologyIRIShortFormProviderTestCase extends TestBase {
    ...
    public void shouldFindLastPathElement() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/", "ont"));
    ...
    public void shouldReturnLastNonNumericPathElement() {
        String shortForm =
            sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/ont/1.1.11", ""));
        assertThat(shortForm, is(equalTo("ont")));
    }
    ...
    public void shouldReturnLastNonVersionPathElement() {
        String shortForm =
            sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/ont/", "v1.1.11"));
        assertThat(shortForm, is(equalTo("ont")));
    }

    @Test
    public void shouldReturnFullIRIForNoPath() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN, ""));
        assertThat(shortForm, is(equalTo(SCHEME_DOMAIN)));
    }

    @Test
    public void shouldStripAwayOWLExtension() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/", "ont.owl"));
        assertThat(shortForm, is(equalTo("ont")));
    }

    @Test
    public void shouldStripAwayRDFExtension() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/", "ont.rdf"));
        assertThat(shortForm, is(equalTo("ont")));
    }

    @Test
    public void shouldStripAwayXMLExtension() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/", "ont.xml"));
        assertThat(shortForm, is(equalTo("ont")));
    }

    @Test
    public void shouldStripAwayOBOExtension() {
        String shortForm = sfp.getShortForm(df.create(SCHEME_DOMAIN + "/ontologies/", "ont.obo"));
        assertThat(shortForm, is(equalTo("ont")));
    }

    @Test
    public void shouldReturnSkosForSKOSNamespace() {
        String shortForm = sfp.getShortForm(df.create("http://www.w3.org/2004/02/skos/", "core"));
        assertThat(shortForm, is(CoreMatchers.equalTo("skos")));
    }

    @Test
    public void shouldReturnDcForDublinCoreNamespace() {
        String shortForm = sfp.getShortForm(df.create("http://purl.org/dc/elements/1.1", ""));
