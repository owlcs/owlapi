...
import static org.semanticweb.owlapi6.model.AxiomType.SUB_PROPERTY_CHAIN_OF;
import static org.semanticweb.owlapi6.model.AxiomType.SWRL_RULE;
...
import static org.semanticweb.owlapi6.vocab.OWLRDFVocabulary.OWL_THING;
...
import org.semanticweb.owlapi6.rdf.model.RDFGraph;
import org.semanticweb.owlapi6.rdf.model.RDFTranslator;
import org.semanticweb.owlapi6.utility.AxiomAppearance;
import org.semanticweb.owlapi6.utility.AxiomSubjectProviderEx;
import org.semanticweb.owlapi6.utility.IndividualAppearance;
import org.semanticweb.owlapi6.utility.OWLAnonymousIndividualsWithMultipleOccurrences;
import org.semanticweb.owlapi6.utility.OWLObjectDesharer;
import org.semanticweb.owlapi6.utility.SWRLVariableExtractor;
...
public abstract class RDFRendererBase {
    ...
    protected static final Set<IRI> prettyPrintedTypes = asUnorderedSet(Stream.of(OWL_CLASS, OWL_OBJECT_PROPERTY,
        OWL_DATA_PROPERTY, OWL_ANNOTATION_PROPERTY, OWL_RESTRICTION, OWL_THING, OWL_NOTHING, OWL_ONTOLOGY,
        OWL_ANNOTATION_PROPERTY, OWL_NAMED_INDIVIDUAL, RDFS_DATATYPE, OWL_AXIOM, OWL_ANNOTATION).map(HasIRI::getIRI));
    ...
    protected RDFRendererBase(OWLOntology ontology, @Nullable OWLDocumentFormat format, OntologyConfigurator config) {
        ...
        desharer = new OWLObjectDesharer(df);
    ...
    private void renderOntologyComponents() {
        renderInOntologySignatureEntities(ontology.determineIllegalPunnings(shouldInsertDeclarations()));
    ...
    private void renderEntities(Stream<? extends OWLEntity> entities, String bannerText, Collection<IRI> illegalPuns) {
        ...
        entities.sorted().filter(e -> createGraph(e, illegalPuns)).forEach(e -> render(e, firstRendering, bannerText));
    }
    ...
    private List<OWLAxiom> getGeneralAxioms() {
        ...
        add(generalAxioms, ontology.axioms(DISJOINT_CLASSES).filter(ax -> ax.getOperandsAsList().size() > 2));
        add(generalAxioms, ontology.axioms(DISJOINT_OBJECT_PROPERTIES).filter(ax -> ax.getOperandsAsList().size() > 2));
        add(generalAxioms, ontology.axioms(DISJOINT_DATA_PROPERTIES).filter(ax -> ax.getOperandsAsList().size() > 2));
    ...
    protected void createGraph(List<? extends OWLObject> objects) {
        ...
        RDFTranslator translator = new RDFTranslator(ontology.getOWLOntologyManager(), ontology, format,
            shouldInsertDeclarations(), occurrences, axiomOccurrences, nextBlankNodeId, blankNodeMap, translatedAxioms);
     ...
     * @param root
     *        true if this is the root call to render, false otherwise
     */
    ...
    static final class GraphVisitor implements OWLEntityVisitor {
        ...
        public void visit(OWLClass cls) {
            add(axioms, ontology.axioms(cls).filter(GraphVisitor::threewayDisjoint).filter(ax -> disjoint(ax, cls))
                .filter(ax -> equiv(ax, cls)));
        ...
        static boolean equiv(OWLAxiom ax, OWLObjectProperty cls) {
            if (ax instanceof OWLEquivalentObjectPropertiesAxiom) {
                OWLEquivalentObjectPropertiesAxiom invAxiom = (OWLEquivalentObjectPropertiesAxiom) ax;
                if (!invAxiom.getOperandsAsList().get(0).equals(cls)) {
                    return false;
                }
            }
